...
void Foam::interfaceProperties::calculateK()
{
    ...
    Info << "Lafaurie smoothening the alpha1 field" << endl;
...
Foam::interfaceProperties::sigmaK_smoothened() const
{
    return fvc::interpolate(sigmaPtr_->sigma())*Kf_;
}


Foam::tmp<Foam::surfaceScalarField>
Foam::interfaceProperties::surfaceTensionForce_smoothened() const
{
    return sigmaK_smoothened()*( (fvc::interpolate(1 - alpha1_)*fvc::snGrad(alpha1_)) - (fvc::interpolate(alpha1_)*fvc::snGrad(1 - alpha1_)) );
}
